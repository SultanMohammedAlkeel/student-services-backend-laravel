                    ...
                    <div class="mb-5">
                        ...
                        <table class="table table-hover shadow-inset rounded">
                            <tr>
                                ...
                                <th class="border-0" scope="col" id="admin">الاسم</th>
                            ...
                            <tbody id="tbody">
                                ...
                                <tr class="table-row">
                                    ...
                                    <th scope="row" headers="admin">{{ $admin->name }}</th>
                                    <td headers="contact">{{ $admin->phone_number }}</td>
                                    <td headers="contact">{{ $admin->email }}</td>
                                    <td headers="college">{{ $roles->where('id', $admin->role_id)->first()->name }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <div class="card-body">
                            <form method="post" action="{{ route('user.store') }}" enctype="multipart/form-data" >
                                ...
                                <div class="form-group">
                                    <div class="input-group mb-4">
                                        <input class="form-control left-text" name="email" placeholder="البريد الإلكتروني"
                                            aria-label="Input group" type="email" value="{{ old('email') }}">
                                        <div class="input-group-append">
                                ...
                                <div class="form-group">
                                    <div class="input-group mb-4">
                                        <input class="form-control" name="password"
                                            placeholder="كلمة المرور" type="password" aria-label="Password" value="{{ old('password') }}">
                                        <div class="input-group-append">
                                ...
                                <div class="form-group">
                                    <div class="input-group mb-4">
                                        <input class="form-control" name="phone_number" placeholder="رقم الهاتف"
                                            aria-label="Input group" type="tel" value="{{ old('phone_number') }}">
                                        <div class="input-group-append">
                                ...
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" name="image_url" aria-label="File upload" id="customFile" accept="image/*">
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, admin) {
                            $('#tbody').append(
                                ...
                                <tr  class="table-row">
                                    <th scope="row">${admin.id}</th>
                                    <th scope="row" headers="admin">${admin.admin_name}</th>
                                    <td headers="college">${admin.college_name}</td>
                                    <td headers="contact">${ admin.short_name }</td>
                                </tr>
                                `
                            );
                        });
